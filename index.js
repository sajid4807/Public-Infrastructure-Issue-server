...
async function run() {
  try {
    ...
    app.get("/staff/stats", verifyFBToken, async (req, res) => {
      try {
        ...
        const working = await reportCollection.countDocuments({
          staffEmail,
          status: "working",
        });
        ...
        const closed = await reportCollection.countDocuments({
          staffEmail,
          status: "closed",
        });

        // 📊 Monthly Issue Chart
        const monthlyIssues = await reportCollection
          .aggregate([
            { $match: { staffEmail } },
            {
              $addFields: {
        ...
        // Fetch today's assigned issues
        const todaysIssues = await reportCollection
          .find({
            staffEmail,
            assignAt: { $gte: todayStart, $lte: todayEnd },
          })
          .toArray();
    ...
    app.post(
      ...
      async (req, res) => {
        ...
        if (!citizenId || !email) {
          return res
            .status(400)
            .send({ message: "citizenId or email missing" });
        }
      ...
      async (req, res) => {
        try {
          ...
          if (!reportId || !email) {
            return res
              .status(400)
              .send({ message: "reportId or email missing" });
          }
